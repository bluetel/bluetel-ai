...
- [ ] T001 Reproduce the defect and record the exact refusal: with a database attached, attempt
      `admin.profiles.setEnabled(true)` on a profile with an enabled bundle and ≥1 workspace entry, and confirm
      it fails with `CONFLICT` carrying `no repository reachability checker configured`, one line per entry.
      Paste the message into the PR description as the before-state.
- [ ] T002 [P] Attach a database and confirm the enable suites are **not** skipping:
...
- [ ] T003 [P] Capture the pre-change reference sweep for comparison at T025:
...
- [ ] T004 [P] Delete the seam and its test:
...
- [ ] T005 [P] Delete the recording fake and its test:
...
- [ ] T006 Strip the probe from `packages/sisyphus-api/src/server/admin/profile-gate.ts`: remove the
...
- [ ] T007 Collapse the router in `packages/sisyphus-api/src/server/admin/profiles.ts`: delete
...
- [ ] T008 Remove `repositoryReachability` from `SisyphusDependencies` in
...
- [ ] T009 Drop the five dead exports from `packages/sisyphus-api/src/server/admin/index.ts`:
...
- [ ] T010 Update the third caller of the deleted factory:
...
- [ ] T011 **Gate**: `pnpm typecheck` passes across the workspace. Any surviving reference to
...
- [ ] T012 [US1] Rebuild the caller in `packages/sisyphus-api/src/server/admin/profiles.test.ts`: replace
...
- [ ] T013 [US1] Replace the two entry-unreachable enable tests in
...
- [ ] T015 [US2] Rewrite `packages/sisyphus-api/src/server/admin/profile-gate.test.ts` around the six retained
...
- [ ] T016 [US2] In `packages/sisyphus-api/src/server/admin/profiles.test.ts`, add the database-backed refusal
...
- [ ] T017 [P] [US2] Remove `workspace_entry` from
...
- [ ] T018 [P] [US2] Update `apps/sisyphus-admin/src/components/admin/profiles/enable-refusal.test.ts`: delete
      ...
      dropped line.
- [ ] T019 [P] [US2] Restate the `element: 'workspace_entry'` fixture in
...
- [ ] T020 [US3] Run `pnpm nx test sisyphus-executor -- workspace` and confirm three guarantees are intact and
...
- [ ] T021 [P] Rewrite the doc comment in `packages/sisyphus-api/src/server/notify/emitter.ts` that cites
...
- [ ] T022 [P] Rewrite the same stale precedent in
      `apps/sisyphus-control-plane/src/jobs/prompt-redact.ts`, which cites
      `createRefusingReachabilityProbe` by name.
- [ ] T023 Delete the `"!src/server/admin/reachability-fake.ts!"` production-only exclusion from the
...
- [ ] T024 Confirm nothing was over-deleted: run `pnpm knip` and verify `describeWorkspaceEntry` in
...
- [ ] T025 Run the reference sweep and diff it against T003's baseline:
...
- [ ] T026 Run the full blocking gate as CI does: `pnpm typecheck`, `pnpm lint:check`, `pnpm knip:orphans`,
      ...
      (Constitution IV).
- [ ] T027 Work through [quickstart.md](./quickstart.md) scenarios 1–5 end to end and confirm each expectation,
