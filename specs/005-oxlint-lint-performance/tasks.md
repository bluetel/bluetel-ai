...
## Phase 9: Amendment — wire the editor to the tools that actually own the rules

**Goal**: PR #40 feedback, and the reason the whole migration was reverted: every command-line path moved
to oxlint, but `.vscode/settings.json` was never touched. It still ran `source.fixAll.eslint` on save and
formatted through Prettier, so the editor was the one place in the workspace where the migration had not
happened — paying the ~5.7 s ESLint invocation the feature exists to remove, in exchange for fix-on-save
over 4 rules of which none is auto-fixable, and reporting a rule set 142 rules short of `pnpm lint:fast`.
Nothing in the lint or format enforcement changes here; the hook and CI paths are byte-for-byte as Phase
8 left them.

- [x] **T065** Point fix-on-save at oxlint: `source.fixAll.oxc` replaces `source.fixAll.eslint` in
      `editor.codeActionsOnSave`, plus `oxc.enable.oxlint` and `oxc.typeAware: true`. The type-aware flag
      is what makes the editor's rule set match `pnpm lint:fast` — unset, the extension defers to
      `.oxlintrc.json`, which does not set `options.typeAware`, so the editor would run 41 fewer rules
      than every other path and look identical while doing it.
- [x] **T066** Add `oxfmt` (0.63.0) and `.oxfmtrc.json`, generated by `oxfmt --migrate prettier` from
      `.prettierrc.mjs` — including `sortTailwindcss`, oxfmt's equivalent of the
      `prettier-plugin-tailwindcss` the Prettier config loads. `oxfmt --list-different .` over the
      repository is empty, which is the only reason the editor can format with oxfmt while `lint-staged`
      still runs `prettier --write`: the two cannot fight over a file they format identically.
- [x] **T067** Scope oxfmt per-language (`javascript`, `javascriptreact`, `typescript`,
      `typescriptreact`, `json`, `jsonc`) rather than as `editor.defaultFormatter`. The extension
      registers for 21 languages, and on the two in this repository outside that list — Markdown and
      `.qlty/qlty.toml` — its output does **not** match Prettier's (nested-list indentation) or Prettier
      cannot parse the file at all (TOML). Those stay with Prettier, and `.oxfmtrc.json`'s
      `ignorePatterns` keeps a bare `oxfmt .` out of them too.
- [x] **T068** Recommend `oxc.oxc-vscode` in `.vscode/extensions.json`, and document the split in
      `AGENTS.md` under Linting → "In the editor". Both language servers are the workspace's own
      binaries (`oxlint --lsp`, `oxfmt --lsp`), so `pnpm install` is what makes the editor work — an
      extension with no local install is a third way for the editor to silently enforce nothing.

**Checkpoint**: `pnpm lint:check`, `pnpm typecheck`, `pnpm test` green; `oxfmt --list-different .` and
`prettier --check` both clean on the same tree; `.husky/pre-commit`, `.github/workflows/ci.yml`,
`.oxlintrc.json` and the ESLint layer unchanged.
