/**
 * MCP task handler for orchestrating task execution.
 *
 * Parses and validates MCP task input, then executes the full pipeline:
...
/**
 * Parses and validates MCP task input from tool arguments.
 *
 * Extracts `repoUrl`, `baseBranch`, `installScript`, and `prompt` from
 * the structured arguments object. Returns a validated `MCPTaskInput`
 * or an `MCPValidationError` with field-specific details.
 *
 * @internal Exported for testing.
 */
export const parseTaskInput = (
  args: Record<string, unknown>,
): MCPTaskInput | MCPValidationError => {
  // Validate repoUrl
...
/**
 * Type guard to distinguish MCPValidationError from MCPTaskInput.
 */
export const isValidationError = (
  result: MCPTaskInput | MCPValidationError,
): result is MCPValidationError => 'valid' in result && !result.valid
...
/**
 * Creates an MCP task handler bound to the given dependencies.
 *
 * @param deps - Shared components: repoCloner, executorRouter, taskStore,
 *   jobQueue, authResult, config, logger
 * @returns MCPTaskHandler with parseTaskInput, executeTask, and cancelTask
 */
export const createMCPTaskHandler = (deps: {
  repoCloner: RepoClonerMCP
  ...
  const executeTask = async (task: MCPTask): Promise<void> => {
    const log = logger.child({
      taskId: task.id,
      repo: task.repoFullName,
      component: 'mcp-task-handler',
    })
    ...
    try {
      ...
      log.info('MCP task execution started')
      ...
      log.info('MCP task completed successfully')
    } catch (error) {
      // Catch-all for unexpected errors
      const message = error instanceof Error ? error.message : String(error)
      log.error({ error: message }, 'Unexpected error during MCP task execution')
      taskStore.setError(task.id, 'kiro-cli', message)
      await generateResultSummary(task, summarizer, taskStore, log)
      taskStore.updateStatus(task.id, 'failed')
    } finally {
  ...
  const cancelTask = (taskId: string): Promise<boolean> => {
    const log = logger.child({ taskId, component: 'mcp-task-handler' })
