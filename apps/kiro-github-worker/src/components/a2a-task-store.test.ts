...
// Feature: rocky-a2a-mode, Property 6: Task status round-trip

import * as fc from 'fast-check'
import pino from 'pino'
import { describe, expect, it } from 'vitest'

import type { A2ATaskInput, A2ATaskStatus } from '../lib/a2a-types'
...
/** All valid A2A task status values. */
const allStatuses: A2ATaskStatus[] = ['submitted', 'working', 'completed', 'failed', 'canceled']

/** Arbitrary that generates a random valid A2ATaskStatus. */
const statusArb: fc.Arbitrary<A2ATaskStatus> = fc.constantFrom(...allStatuses)
...
/** Arbitrary that generates valid A2ATaskInput objects. */
const taskInputArb: fc.Arbitrary<A2ATaskInput> = fc.record({
  repoUrl: fc
    .tuple(ownerArb, repoNamePartArb)
...
// ── Property 8: A2A queue key derivation format ──
// **Validates: Requirements 8.1**

describe('Property 8: A2A queue key derivation', () => {
  it('derived queue key equals {repoFullName}:a2a-{taskId} for any valid input', () => {
    fc.assert(
      fc.property(taskInputArb, repoFullNameArb, (input, repoFullName) => {
        const store = createA2ATaskStore(mockLogger)

        const task = store.create(input, repoFullName)

        expect(task.queueKey).toBe(`${repoFullName}:a2a-${task.id}`)
      }),
      { numRuns: 100 },
    )
  })
...
// ── Property 6: Task status update round-trip ──
// **Validates: Requirements 7.1, 7.2**

describe('Property 6: Task status round-trip', () => {
