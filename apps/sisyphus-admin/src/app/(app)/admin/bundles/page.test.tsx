...
const listQuery = vi.fn((): unknown => ({ data: undefined, isPending: true, error: null }))
...
describe('the bundles page', () => {
  ...
  it('reports a refused list read instead of claiming the shelf is empty (FR-201)', () => {
  ...
  it('is built from tokens only (SC-015)', () => {
    const markup = renderToStaticMarkup(<BundlesPage />)
  ...
  it('opens no main landmark and sets no column of its own — the shell owns both (T156)', () => {
    // The page column and the `<main>` moved to `src/app/(app)/layout.tsx`, which wraps every
    // authenticated screen. A second one here would nest a main inside a main and a column inside a
    // column.
    const markup = renderToStaticMarkup(<BundlesPage />)
