 ...
 * refuse to run: the application config resolves its values from the parameter
 * store before it can name the app, and the bootstrap config expects the ambient
 * credential chain. Neither precondition holds during `pnpm install`. This file
 * has no preconditions at all, which is the whole point of it (FR-198, FR-199).
 *
 * ---------------------------------------------------------------------------
 ...
 * API and runs against another. Change one, change all four: `sst.config.ts`,
 * `sst-bootstrap.config.ts`, this file, and the matching pin in
