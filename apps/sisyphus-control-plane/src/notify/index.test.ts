 ...
 * barrel that did would make `import { … } from './notify'` a side effect, and the seam-plus-fake
 * arrangement in `../aws/` exists precisely so that importing costs nothing.
 *
 * **It does not export the fixture seeder.** `notify-fixtures.ts` is live-database test support;
 * exporting it would put it one import away from the delivery path, the same rule
 * `../jobs/index.ts` keeps for `workflow-fixtures.ts`.
 */
