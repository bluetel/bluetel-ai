/**
 * The Slack seam — what the control plane needs from Slack, and nothing else (T084, FR-136).
 *
 ...
 * interface exposes two, for the same reason `../aws/` exposes twelve across four services — a seam
 * the width of the SDK is a re-export, and the delivery path would be untestable without a real
 * workspace and a real token.
 *
 * **No module in this directory constructs a client.** The adapter is handed one, so importing the
