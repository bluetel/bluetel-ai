 ...
 * and linter can see it — but nothing else under `src/notify/` imports it, and it is deliberately
 * **not** re-exported from `./index.ts`, exactly as `../jobs/workflow-fixtures.ts` is kept out of
 * `../jobs/index.ts`. Exporting it would put a fixture seeder one import away from the delivery
 * path.
 *
